...
## Phase 17: Commercial SaaS Readiness

Phase 17 is a final planned readiness phase, not an automatic commercial launch. `COMMERCIAL_SAAS_ENABLED=false`, `COMMERCIAL_BILLING_ENABLED=false`, and `COMMERCIAL_BILLING_PROVIDER=disabled` are the safe defaults. The server gate also requires a stored readiness checklist covering tenant isolation, billing review, export testing, deletion testing, rules testing, and support-access review before commercial mode can be enabled. Completing onboarding or seeding a plan never activates billing or migrates internal users.

SourceHub uses immutable workspace IDs as the tenant boundary for this compatibility layer. New commercial records carry both `tenantId` and `workspaceId` with the same stable value. Tenant context is resolved from an authenticated membership and an HttpOnly active-tenant cookie; tenant names, email domains, and slugs are never security boundaries. Existing internal modules continue to use the existing default workspace while commercial mode is disabled. A destructive migration is not performed. Future migration work must use a resumable, checkpointed mapping job and must be reviewed before existing records are rewritten.

The commercial service in `lib/commercial.ts` owns provisioning, owner membership, invitations, branding, onboarding, entitlements, lifecycle transitions, usage events, billing projections, support-session requests, integrations, tenant API credentials, custom-domain readiness records, exports, import dry runs, and delayed deletion scheduling. Firestore and Storage rules require explicit `tenantId` claims for commercial client reads and deny direct commercial writes. All sensitive mutations are Admin SDK/server-only operations and create audit evidence. Tenant API credentials are hashed and returned once; integration secret values are represented only by server-side references.

Plans and plan versions are stored separately from tenant subscriptions. Effective entitlements are resolved server-side from the subscription plan, version, tenant overrides, and feature flags. Initial seed records include only an internal sandbox plan with no commercial pricing promise. Usage metrics are centrally named, recorded through idempotent server events, aggregated into bounded daily/monthly documents, and retained by scheduled Functions. Quotas are designed to warn and preserve existing data rather than delete records when limits change.

Billing is an abstraction only. No Stripe or other live provider is enabled. Hosted checkout and portal actions remain blocked while billing is disabled. The billing webhook endpoint accepts only a configured HMAC signature, stores provider event IDs for duplicate rejection, applies validated lifecycle transitions, and keeps a local subscription projection. Raw card details are never collected or stored. This code does not claim PCI, tax, accounting, POPIA, GDPR, or legal compliance.

Onboarding is available at `/onboarding`, platform operations at `/administration/saas`, and the developer portal at `/developers`. The public OpenAPI document remains at `/api/v1/openapi`; API credentials derive tenant scope from the server-side credential and cannot select another tenant by request parameter. Custom domains only create a requested/DNS-instructions record when explicitly enabled; there is no DNS or certificate automation. The two catalogued integrations are SourceHub-owned sandbox contracts, not speculative third-party connectors.
