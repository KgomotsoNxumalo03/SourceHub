...
async function main() {
  ...
  // Enterprise readiness records are deliberately disabled or not configured until cloud controls are approved.
  await upsert(collectionNames.enterpriseIdentityProviders, `${workspaceId}:google`, { id: `${workspaceId}:google`, workspaceId, name: "Google Workspace readiness", providerType: "GOOGLE", enabled: false, allowedEmailDomains: ["sourceitservices.example"], tenantId: "", authority: "", issuer: "", clientId: "", redirectUri: "", claimMappings: {}, defaultRole: "Employee", defaultPermissionProfile: "", autoCreateUsers: false, allowExistingEmailLinking: false, groupMappings: {}, loginEnforcement: "OPTIONAL", secretRef: "" });
  await upsert(collectionNames.enterpriseIdentityProviders, `${workspaceId}:entra`, { id: `${workspaceId}:entra`, workspaceId, name: "Microsoft Entra ID readiness", providerType: "MICROSOFT_ENTRA", enabled: false, allowedEmailDomains: ["sourceitservices.example"], tenantId: "", authority: "", issuer: "", clientId: "", redirectUri: "", claimMappings: {}, defaultRole: "Employee", defaultPermissionProfile: "", autoCreateUsers: false, allowExistingEmailLinking: false, groupMappings: {}, loginEnforcement: "OPTIONAL", secretRef: "" });
  await upsert(collectionNames.enterpriseMfaPolicies, `${workspaceId}:default`, { id: `${workspaceId}:default`, workspaceId, name: "Enterprise MFA readiness", enabled: false, appliesTo: "HIGH_RISK_ACTIONS", roleNames: [], userIds: [], factorPolicy: "EITHER", recoveryEnabled: false });
  for (const [id, name, code, address, timeZone] of [["office-jhb", "Johannesburg", "JHB", "Fictional Source IT Services Johannesburg office", "Africa/Johannesburg"], ["office-cpt", "Cape Town", "CPT", "Fictional Source IT Services Cape Town office", "Africa/Johannesburg"], ["office-dbn", "Durban", "DBN", "Fictional Source IT Services Durban office", "Africa/Johannesburg"], ["office-remote", "Remote", "REMOTE", "Approved remote working location", "Africa/Johannesburg"]]) await upsert(collectionNames.offices, id, { id, workspaceId, name, code, address, latitude: null, longitude: null, timeZone, contactEmail: "", contactNumber: "", managerId: "", defaultWorkingHours: { start: "08:00", end: "17:00", days: ["MON", "TUE", "WED", "THU", "FRI"] }, active: true, status: "ACTIVE", createdBy: adminId, updatedBy: adminId });
  await upsert(collectionNames.backupPolicies, `${workspaceId}:default`, { id: `${workspaceId}:default`, workspaceId, name: "Managed Firestore backup readiness", status: env.ENTERPRISE_BACKUP_STATUS, schedule: "daily", retentionDays: 35, targetProject: env.ENTERPRISE_BACKUP_PROJECT || null, targetBucket: env.ENTERPRISE_BACKUP_BUCKET || null, restoreTestFrequency: "quarterly", lastVerifiedAt: null, createdBy: adminId, updatedBy: adminId });
  await upsert(collectionNames.securityPolicies, `${workspaceId}:default`, { id: `${workspaceId}:default`, workspaceId, name: "Enterprise security baseline", status: "READINESS", controls: { identityProviders: false, mfa: false, apiKeys: true, webhooks: true, backups: env.ENTERPRISE_BACKUP_STATUS === "CONFIGURED", appCheck: env.FIREBASE_APP_CHECK_ENABLED }, reviewedAt: now, reviewedBy: adminId });
  await upsert(collectionNames.featureFlags, `${workspaceId}:enterprise`, { id: `${workspaceId}:enterprise`, workspaceId, name: "enterprise.identity.readiness", description: "Controls the enterprise identity readiness surface.", officeId: "", roleNames: [], userGroup: "", environment: "all", enabled: env.ENTERPRISE_ENABLED, rolloutPercentage: 100, emergencyDisabled: false, startsAt: now, endsAt: null, createdBy: adminId, updatedBy: adminId });
  await upsert(collectionNames.enterpriseAuditEvents, `${workspaceId}:seed`, { id: `${workspaceId}:seed`, workspaceId, actorId: adminId, action: "enterprise.readiness.seeded", targetType: "EnterpriseReadiness", targetId: workspaceId, result: "SUCCESS", reason: "Fictional development readiness records created.", correlationId: "seed", metadata: { secretsStored: false, backupsConfigured: env.ENTERPRISE_BACKUP_STATUS === "CONFIGURED" }, createdAt: now, expiresAt: new Date("2033-07-22T00:00:00.000Z") });

  // Commercial SaaS is seeded as an internal sandbox only. No live billing or external tenant activation is implied.
  await upsert(collectionNames.commercialTenants, workspaceId, { id: workspaceId, tenantId: workspaceId, workspaceId, name: env.DEFAULT_WORKSPACE_NAME, lifecycleState: "ACTIVE", isInternal: true, ownerId: adminId, createdAt: now, updatedAt: now });
  await upsert(collectionNames.tenantMemberships, `${workspaceId}:${adminId}`, { id: `${workspaceId}:${adminId}`, tenantId: workspaceId, workspaceId, userId: adminId, roleKey: "OWNER", status: "ACTIVE", joinedAt: now, createdAt: now, updatedAt: now });
  await upsert(collectionNames.commercialReadiness, workspaceId, { id: workspaceId, tenantId: workspaceId, workspaceId, enabled: false, checklist: { tenantIsolationReviewed: false, billingProviderReviewed: false, exportsTested: false, deletionTested: false, rulesTested: false, supportAccessReviewed: false }, notes: "Commercial mode remains disabled until the internal readiness review is complete.", updatedBy: adminId, updatedAt: now });
  await upsert(collectionNames.commercialPlans, "internal-sandbox", { id: "internal-sandbox", key: "internal-sandbox", name: "Internal Sandbox", description: "Development-only internal SourceHub plan; not a commercial offer.", status: "PUBLISHED", public: false, version: 1, currency: env.FINANCE_DEFAULT_CURRENCY, createdBy: adminId, updatedAt: now });
  await upsert(collectionNames.commercialPlanVersions, "internal-sandbox:v1", { id: "internal-sandbox:v1", planKey: "internal-sandbox", version: 1, status: "PUBLISHED", entitlements: { "sourcehub.internal": { enabled: true, limit: null, source: "PLAN" }, "api.access": { enabled: true, limit: 120, unit: "requests/minute", source: "PLAN" }, "members.active": { enabled: true, limit: null, unit: "members", source: "PLAN" } }, limits: { activeMembers: null, storageBytes: null, automationRunsMonthly: null, aiRequestsMonthly: null }, effectiveFrom: now, createdBy: adminId });
  for (const [id, name, unit] of [["active_members", "Active members", "members"], ["storage_bytes", "Storage", "bytes"], ["api_requests", "API requests", "requests"], ["automation_runs", "Automation runs", "runs"], ["ai_requests", "AI requests", "requests"], ["integration_executions", "Integration executions", "executions"]]) await upsert(collectionNames.commercialFeatures, id, { id, key: id, name, unit, status: "AVAILABLE", description: "Central commercial usage metric definition.", createdBy: adminId, updatedAt: now });
  await upsert(collectionNames.commercialSubscriptions, workspaceId, { id: workspaceId, tenantId: workspaceId, workspaceId, planKey: "internal-sandbox", planVersion: 1, lifecycleState: "ACTIVE", provider: "disabled", currency: env.FINANCE_DEFAULT_CURRENCY, createdAt: now, updatedAt: now });
  await upsert(collectionNames.commercialOnboarding, workspaceId, { id: workspaceId, tenantId: workspaceId, workspaceId, currentStep: "completion", completedSteps: ["organization", "office", "branding", "members", "security", "integrations", "completion"], skippedSteps: [], essentialModules: ["service-desk", "assets", "reports"], status: "READY", createdAt: now, updatedAt: now });
  await upsert(collectionNames.commercialIntegrations, "sourcehub-webhook", { id: "sourcehub-webhook", name: "SourceHub Webhooks", category: "Developer tools", version: "1.0.0", scopes: ["webhooks.write"], status: "SANDBOX", documentationUrl: "/developers#webhooks", platformManaged: true });
  await upsert(collectionNames.commercialIntegrations, "sourcehub-import-export", { id: "sourcehub-import-export", name: "SourceHub Data Portability", category: "Data", version: "1.0.0", scopes: ["data.read", "data.write"], status: "READY", documentationUrl: "/developers#portability", platformManaged: true });
  await upsert(collectionNames.commercialChangelog, "phase-17", { id: "phase-17", version: "2026.07", title: "Commercial SaaS readiness foundation", status: "SANDBOX", summary: "Tenant, entitlement, billing, usage, onboarding, branding, integration, and lifecycle foundations are available behind a disabled server gate.", publishedAt: now });

  console.log("Seeded SourceHub workspace, CRM, service desk, asset, network, employee, attendance, project, finance, knowledge, reporting, automation, AI, mobile, enterprise, and commercial sandbox readiness data.");
}
